...
describe('Reading unit 5 literal content', () => {
  ...
  test('coaches central-idea evidence with Winnie-sized strategy and retry copy', () => {
    const card = unit05Lessons[0]!.learnCards[1]!;
    expect(card.widgetCoach?.intro).toHaveLength(2);
    expect(card.widgetCoach?.reactions.strategy?.text).toContain('complete passage');
