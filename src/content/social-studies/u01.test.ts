...
const titles = ["When Cultures Met", "Trade and Forced Labor", "Life in Three Colonial Regions", "People of Early South Carolina", "How Colonial Work Changed", "Clues to Colonial Life"];
const activities = ["history-evidence-board", "history-cause-effect", "history-map", "history-map", "history-timeline", "history-evidence-board"];
const indicators = ['CO', 'CE', 'P', 'CX', 'CC', 'E'];

test('unit 1 preserves the approved six lesson identities and activity allocation', () => {
  expect(unit01Lessons).toHaveLength(6);
  unit01Lessons.forEach((lesson, index) => {
    expect(lesson.id).toBe(`social-studies-u01-l${String(index + 1).padStart(2, '0')}`);
    expect(lesson.title).toBe(titles[index]);
    expect(lesson.indicatorCodes).toEqual([`4.1.${indicators[index]}`]);
    expect(lesson.learnCards[1].widget?.type).toBe(activities[index]);
  });
});

test('unit 1 offers supported review cards, varied questions, and complete coaching', () => {
