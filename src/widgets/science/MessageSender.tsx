...
function effectiveAlphabet(encoding: 'morse' | 'binary', overrides: Record<string, string> | undefined) {
  const normalizedOverrides = Object.fromEntries(Object.entries(overrides ?? {}).map(([character, code]) => [encoding === 'morse' ? character.toUpperCase() : character, code]));
...
export default function MessageSender({ config, onEvent }: WidgetProps<'message-sender'>) {
  ...
  const message = config.encoding === 'morse' ? config.message.trim().toUpperCase() : config.message.trim();
  ...
  const append = (symbol: string) => {
    setSent(false);
    setStatus('Message changed; send it when ready.');
  ...
  return <section className="card widget-experiment message" data-testid="widget-message-sender" data-state={visiblyComplete ? 'complete' : 'encoding'} data-encoding={config.encoding}>
    ...
    <section className="message-reference" aria-label="Code reference">
      ...
      <ul>{relevantCharacters.map((character) => <li key={character}>{character} = {alphabet[character]}</li>)}</ul>
    </section>
    <div className="message-entry" role="group" aria-label={`Entered code grouped by character: ${enteredGroups.join(' separator ') || 'empty'}`}>
      {enteredGroups.map((group, index) => <span key={`${group}-${index}`}><span data-testid="encoded-character-group" className="encoded-character-group">{group || '…'}</span>{index < enteredGroups.length - 1 && <span data-testid="encoded-character-separator" className="encoded-character-separator" aria-label="letter separator">|</span>}</span>)}
    </div>
    <div className="message-controls" aria-label="Code symbol controls">
      ...
      <button aria-label="Add letter separator" disabled={!encoded || encoded.endsWith(' ')} onClick={() => append(' ')}>Separate letters</button>
